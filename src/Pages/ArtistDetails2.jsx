...
export default function ArtistDetails2() {
...
const [artistRednderDetails, setartistRednderDetails] = useState([]);
...
const [topAlbums, setTopAlbums] = useState([])
  ...
  const options = {
    ...
    async function getArtistSummary (){
      ...
      let artistResponce = await responce.resources.artists ;
      let artistDeatils = [];
    //   setImageSourcrRender(responce.resources.artists.artist_id.attributes?.artwork.slice(0,responce.resources.artists.artist_id.attributes?.artwork.length - 13).concat("592x592bb.jpg")        )
      for (const artist in artistResponce) {
        artistDeatils.push( [ artistResponce[artist].id , 
                      artistResponce[artist].attributes.name, 
                      (artistResponce[artist].attributes.artwork.url).slice(0,artistResponce[artist].attributes.artwork.url.length - 13).concat("592x592bb.jpg"),
                    ])
      }
      setartistRednderDetails(artistDeatils[0]);
      console.log(artistDeatils);
      let songs = [];
    ...
    <div className='mb-[100px]'>
                <div className=''   >
                      <div className='md:h-[250px] h-[150px] flex justify-center items-center'> 
                          <img className='h-[80%]  rounded-full shadow-md' src={artistRednderDetails[2]} alt={artistRednderDetails[1]} />
                      </div>
                      <div className='text-center'> <span className='artistCoverTitle'> {artistRednderDetails[1]} </span> </div>
                </div>
